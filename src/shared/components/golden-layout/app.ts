...
export class App {
    ...
    constructor(jsxComponents: (JSX.Element | (()=>JSX.Element))[], cfg?: {
        useVirtualEventBinding?: boolean // if true, Golden Layout tries to minimize, but introduce artifact when maximized
    }) {
        ...
        (this._goldenLayout as any).beforeResizingEvent = (count: any) => this.handleBeforeResizingEvent(count);
    ...
    private handleLoadLayoutButtonClick() {
        ...
        const selectedLayout = layouts.find((layout: any) => layout.name === layoutName);
    ...
    goldenLoadPredefinedLayout(layoutId: string) {
        const layoutName = layoutId;
        const layouts = prefinedLayouts.allComponents;
        const selectedLayout = layouts.find((layout: any) => layout.name === layoutName);
        if (selectedLayout === undefined) {
            console.log("Failed to Load predefined layout. Available layout IDs are : ", layouts.map((layout: any) => layout.name));
            throw new Error('handleLayoutSelectChange');
        } else {
            this._goldenLayout.loadLayout(selectedLayout.config);
        }
